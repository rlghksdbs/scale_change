...
parser.add_argument("--root_folder4", type=str, default='/mnt/hdd12TB/SBS_DATA/SD/vfi_png', help='path of video to be converted')
parser.add_argument("--root_folder5", type=str, default='/mnt/hdd12TB/SBS_DATA/SD/vfi_png', help='path of video to be converted')
parser.add_argument("--root_folder6", type=str, default='/mnt/hdd12TB/SBS_DATA/SD/vfi_png', help='path of video to be converted')
...
parser.add_argument("--video_folder5", type=str, default='/mnt/hdd12TB/SBS_DATA/220607_SBS_db_r1/png/test/SD_AllIn_8th_1', help='path of video to be converted')
parser.add_argument("--video_folder6", type=str, default='/mnt/hdd12TB/SBS_DATA/220607_SBS_db_r1/png/test/SD_AllIn_8th_1', help='path of video to be converted')
...
parser.add_argument("--video_name5", type=str, default='%6d.png', help='path of video to be converted')
parser.add_argument("--video_name6", type=str, default='%6d.png', help='path of video to be converted')
...
def create_video_side_by_side(dir_name1, dir_name2, dir_name3, dir_name4, dir_name5, dir_name6, file_name1, file_name2, file_name3, file_name4, file_name5, file_name6, dir_out, out_name):
    ...
    retn = os.system('{} -r {} -i {}/{} -r {} -i {}/{} -r {} -i {}/{} -r {} -i {}/{} -r {} -i {}/{} -r {} -i {}/{} ' 
                     '-filter_complex "[0]drawtext=text="원본":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v0]; '
                     '[1]drawtext=text="baseline":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v1]; '
                     '[2]drawtext=text="Only_VGG":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v2]; '
                     '[3]drawtext=text="L1_VGG":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v3]; '
                     '[4]drawtext=text="Mag_1":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v4]; '
                     '[5]drawtext=text="Mag_0.1":fontsize=20:x=20:y=10:fontcolor=white:box=1:boxcolor=red:boxborderw=5[v5]; '
                    ...
                    #  args.fps, dir_name2, file_name2, args.fps, dir_name3, file_name3, dir_out, out_name))
                     format(os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir_name1, file_name1,
                     args.fps, dir_name2, file_name2, args.fps, dir_name3, file_name3, args.fps, dir_name4, file_name4, args.fps, dir_name5, file_name5, args.fps, dir_name6, file_name6, dir_out, out_name))
    if retn:
        error = "Error creating output video. Exiting."
    return error
...
if __name__ == '__main__':
    ...
    for (path, last_dir, files) in os.walk(args.root_folder4):
        if not last_dir:
            dir_list4.append(path)
    dir_list4 = sorted(dir_list4)

    dir_list5 = []
    for (path, last_dir, files) in os.walk(args.root_folder5):
        if not last_dir:
            dir_list5.append(path)
    dir_list5 = sorted(dir_list5)

    dir_list6 = []
    for (path, last_dir, files) in os.walk(args.root_folder6):
        if not last_dir:
            dir_list6.append(path)
    dir_list6 = sorted(dir_list6)
    ...
    for dir1 in dir_list1:
        ...
        dir5 = [s for s in dir_list5 if sequence_name in s]
        dir6 = [s for s in dir_list6 if sequence_name in s]
        ...
        if dir2:
            ...
            file_list5 = glob.glob(dir5 + '*.png')
            file_list6 = glob.glob(dir6 + '*.png')
            ...
            if len(file_list1) == len(file_list2):
                ...
                create_video_side_by_side(dir1, dir2, dir4, dir4, dir5, dir6, args.video_name1, args.video_name2, args.video_name3, args.video_name4, args.video_name5, args.video_name6, args.out_folder, out_file_name)
