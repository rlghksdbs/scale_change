...
def create_video_side_by_side(dir_name1, dir_name2, dir_name3, dir_out, out_name):
    error = ""
    #print('{} -r {} -i {}/%d.jpg -qscale:v 2 {}'.format(os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir, args.output))
    retn = os.system('{} -i {} -i {} -i {} -filter_complex "[0:v][1:v][2:v]hstack=inputs=3[v]" -map "[v]" {}/{}'.format(os.path.join(args.ffmpeg_dir, "ffmpeg"), dir_name1, dir_name2, dir_name3, dir_out, out_name))
    ...
    for dir1 in dir_list1:
        ...
        if dir2:
            ...
            if len(file_list1) == len(file_list2):
                ...
                create_video_side_by_side(dir1, dir2, dir3, args.out_folder, out_file_name)
