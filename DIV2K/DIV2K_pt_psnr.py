...
if __name__ == '__main__':
    ...
    rgb_range = 255
    scale = 3
    lr_dir = os.path.join(dir_data, 'DIV2K/bin/DIV2K_train_LR_bicubic/X{}/'.format(scale))
    hr_dir = os.path.join(dir_data, 'DIV2K/bin/DIV2K_train_HR/')
    ...
    files = []
    train_list = open('/data/DIV2K/DIV2K_train_list.txt', 'w')
    train_map = open('/data/DIV2K/DIV2K_train_map.txt', 'w')
    for f in all_files:
        files.append(f)
    files.sort()
    ...
    for i, file in enumerate(files):
        ...
        with open(lr_file, 'rb') as _f:
            ...
            lr_tensor = tf.convert_to_tensor(lr, dtype=np.float32)
        ...
        with open(hr_file, 'rb') as _f:
            ...
            hr_tensor = tf.convert_to_tensor(hr, dtype=np.float32)
            w = hr_tensor.shape[0]
            h = hr_tensor.shape[1]
        ...
        sr_tensor = tf.image.resize(lr_tensor, [w, h])

        #sr_tensor = sr_tensor.squeeze().permute(1, 2, 0)  # (W,H,3)

        sr = sr_tensor.numpy()   #numpyndarray
        ...
        lr_file_list = os.path.join(dir_data, 'DIV2K/bin/DIV2K_train_LR_bicubic/X{}_patch{}_list/'.format(scale, hr_patch_size))
        lr_file_map = os.path.join(dir_data, 'DIV2K/bin/DIV2K_train_LR_bicubic/X{}_patch{}_map/'.format(scale, hr_patch_size))

        if not os.path.exists(lr_file_list):
            os.makedirs(lr_file_list)

        if not os.path.exists(lr_file_map):
            os.makedirs(lr_file_map)
        ...
        #ii_list_file = lr_file_list + file.replace(".pt", ".pt".format(hr_patch_size))
        #ii_map_file = lr_file_map + file.replace(".pt", ".pt".format(hr_patch_size))
        ii_list = file.replace('x{}.pt'.format(scale), '.pt'.format(hr_patch_size))
        ii_map = file.replace('x{}.pt'.format(scale), '.pt'.format(hr_patch_size))
        print("saving {}\t{}".format(ii_list_file, ii_map_file))
        with open(ii_list_file, 'wb') as _fl:
            pickle.dump(index_psnr, _fl)
        train_list.write('{}\n'.format(ii_list))
        with open(ii_map_file, 'wb') as _fm:
            pickle.dump(psnr_map, _fm)
        train_map.write('{}\n'.format(ii_map))
        toc = time.time()
